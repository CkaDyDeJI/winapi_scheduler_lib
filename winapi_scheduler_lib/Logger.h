...
{
public:
	static CLogger<CNoLock>* Instance()
	{
		if (logger == nullptr)
			Logger();
...
private:
	static CLogger <CNoLock>* logger;

	Logger()
	{
		logger = new CLogger<CNoLock>(LogLevel::Info, _T("MyApp"), static_cast<int>(LogItem::DateTime));
